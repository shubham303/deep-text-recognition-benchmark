...
def train(opt):
    ...
    if opt.saved_model != '':
        ...
        if opt.FT:
            model.load_state_dict(torch.load(opt.saved_model, map_location=configuration.device), strict=False)
        else:
            model.load_state_dict(torch.load(opt.saved_model, map_location=configuration.device))
    print("Model:")
    print(model)
    ...
    while(True):
        ...
        # validation part
        if (iteration + 1) % opt.valInterval == 0 or iteration == start_iter: # To see training progress, we also conduct
            # validation when 'iteration == 0'
            ...
            with open(f'./saved_models/{opt.exp_name}/log_train.txt', 'a') as log:
                ...
                model.train()
                #todo save models by language name.
                ...
                print(best_accuracy , "best accuracy")
