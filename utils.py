...
class TransformerLabelConvertor(object):
    ...
    def encode(self, text, batch_max_length=25):
        ...
        for i, t in enumerate(text):
            ...
            #batch_text[:,0]=0 # batch_text[:, 0] = [GO] token
        return (batch_text.to(configuration.device), torch.IntTensor(length).to(configuration.device))
...
def getCharacterList(lang):
    #Note: Ensure list of characters has same order everytime it is created. better to sort the list of characters
    # before returning.
